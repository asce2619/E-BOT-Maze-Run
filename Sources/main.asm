...
LINE_VARIANCE       	FCB   $F6       	; Adding variance based on testing to   High - [(High + Low)/2]
BOW_VARIANCE        	FCB   $42       	; Establish baseline for sensors
PORT_VARIANCE       	FCB   $48                	 
MID_VARIANCE        	FCB   $3F
STARBOARD_VARIANCE  	FCB   $39
...
; subroutine section
;***************************************************************************************************
DISPATCHER    	JSR   START_BOT          ; Start of the Dispatcher
              	RTS

START_BOT     	CMPA  #START             ; Verify if the robot's state is START
              	BNE   START_FWD          ; If not, move to FORWARD state validation
              	JSR   START_ST           ; Validate START state
              	RTS                                    	 

START_FWD     	CMPA  #FWD               ; Verify if the robot's state is FORWARD
              	BNE   ASTOP              ; If not, move to ALL_STOP state validation
              	JSR   FWD_ST             ; Validate FORWARD state
              	RTS
             	 
REV_TURN      	CMPA  #REV_TRN           ; Verify if the robot's state is REV_TURN
              	BNE   L_ALIGN            ; If not, move to LEFT_ALIGN state validation
              	JSR   REV_TRN_ST         ; Validate REV_TURN state
              	RTS                                      	 

ASTOP          	CMPA  #ALL_STOP          ; Verify if the robot's state is ALL_STOP
              	BNE   LEFT_TURN          ; If not, move to LEFT_TURN state validation
...
NO_FWD_BUMP   	BRSET PORTAD0, $08, NO_REAR_BUMP  	; Checks if the stern bumper
              	MOVB	#REV_TRN, CRNT_STATE          ; if true, reverse and enter the maze again             	 
              	JSR 	INIT_REV                      
              	JSR 	UPDT_DISPL                   	; Update the display                           	 
              	JSR 	INIT_REV                                                           	 
              	LDY 	#6000                                                              	 
